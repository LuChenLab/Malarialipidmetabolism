...
```{r}
set.seed(123)
folds <-createMultiFolds(y=balanced_data$Lable,k=5,times=1)
```
##2.2.
```{r,fig.width=5,fig.height=4}
AUROC_Plot <- lapply(c(1:5),function(x){
  train<- balanced_data[ folds[[x]],] 
  test <- balanced_data[-folds[[x]],] 
  set.seed(123)
  rfModel <- randomForest(Lable ~ . - GeneID, data = train, importance = TRUE,proximity = TRUE,ntree = 500)
  model_pre <- predict(rfModel, test,type = "prob")
  model_pre <- model_pre[,2]
  #
  roc_obj <- AUC(obs=test$Lable,pred=model_pre,curve = "ROC", simplif=FALSE,plot = FALSE)
...
AUPRC_Plot <- lapply(c(1:5),function(x){
  train<- balanced_data[ folds[[x]],] 
  test <- balanced_data[-folds[[x]],] 
  set.seed(123)
  rfModel <- randomForest(Lable ~ . - GeneID, data = train, importance = TRUE,proximity = TRUE,ntree = 500)
  ...
  model_pre <- model_pre[,2]
  #
  aupr <- AUC(obs=test$Lable,pred=model_pre,curve = "PR", simplif=FALSE,plot = FALSE)
...
Gini_Plot <- lapply(c(1:5),function(x){
  train<- balanced_data[ folds[[x]],] 
  test <- balanced_data[-folds[[x]],] 
  set.seed(123)
  rfModel <- randomForest(Lable ~ . - GeneID, data = train, importance = TRUE,proximity = TRUE,ntree = 500)
...
ggplot(AUROC_Plot,aes(x=1-specificity,y=sensitivity,group=Time,color=Time))+geom_line(alpha=.8,size=.5)+theme_bw()+
  geom_segment(aes(x = 0, y = 0, xend = 1, yend = 1), colour = "grey",linetype=2)+
  ggtitle("AUROC")+
  theme(axis.text = element_text(size = 12),axis.title = element_text(size = 15),
        plot.title = element_text(size = 20,hjust = 0.5)
        )+coord_fixed(ratio = .9)+guides(color=FALSE)+
...
AUROC_Plot_new$Time <- factor(AUROC_Plot_new$Time,levels = c("Mean","1","2","3","4","5"))
#
...
ggplot(AUPRC_Plot,aes(x=sensitivity,y=precision,group=Time,color=Time))+geom_line(alpha=.8,size=.5)+theme_bw()+
  # scale_x_continuous(expand = c(0.001,0.04))+scale_y_continuous(expand = c(0.001,0.04))+
  geom_segment(aes(x = 0, y = 1, xend = 1, yend = 0), colour = "grey",linetype=2)+
  ggtitle("AUPRC")+
  theme(axis.text = element_text(size = 12),axis.title = element_text(size = 15),
        plot.title = element_text(size = 20,hjust = 0.5)
        )+coord_fixed(ratio = .9)+guides(color=FALSE)+
   annotate(geom = "text",label=paste0("Mean of AUPRC: ",round(mean(AUPRC_Plot$PR),digits = 2)),x=.3,y=.15,size=5)
```
###3.3.2.
```{r}
...
AUPRC_Plot_new$Time <- factor(AUPRC_Plot_new$Time,levels = c("Mean","1","2","3","4","5"))
#
...
```{r,fig.width=5,fig.height=4}
setorder(MRS,-value)
MRS$GeneID <- factor(MRS$GeneID,levels = MRS$GeneID)
ggplot(MRS,aes(x=value))+geom_density()+theme_classic()+ggtitle("Distribution of lipid related MRS")+
...
##4.3. 
```{r}
Pf_gene_ori <- readRDS("../Data/pf_Mfuzzgene.Rds")
Pb_gene_ori <- readRDS("../Data/pb_Mfuzzgene.Rds")
#
